...
**Spotify matching and writing** (`services/spotify/matching.js`, `services/spotify/spotify.match.js`, `services/spotify/spotify.tracks.js`, spec `specs/04-spotify-matching-y-escritura.md`): three endpoints translate a screenshot-detected song into a Spotify track and get it into a playlist. `POST /spotify/match` searches Spotify for each song in the batch (concurrency 5 via `utils/concurrency.js`) and scores every candidate with `matching.js`'s pure `scoreCandidate`/`pickBest` — weighted title/artist/duration/popularity, keyword penalties for live/karaoke/remix, and a forced `ambiguous` status on near-ties (<0.04) or low source `confidence` (<70). `GET /spotify/search` reuses the same track mapping without scoring (`confidence: null`), for manual correction. `POST /spotify/playlists/:playlistId/items` writes the confirmed URIs.

**Deduplication is ours, not Spotify's.** Adding a URI already in a playlist returns `201` and silently duplicates it — there is no duplicate error code to catch. `addTracks` (`spotify.tracks.js`) reads the whole destination playlist first (`readPlaylistUris`, paginated at `limit=100`, confirmed working against the live API despite Spotify's docs contradicting themselves on the max) and filters incoming URIs against that `Set` — matches go to `skippedDuplicates` — and against themselves, so the same URI sent twice in one body is written once.

**The write path disables `spotifyFetch`'s default 5xx retry** (`{ retryServerErrors: false }`) — a 5xx on write can mean the write actually landed and the response was lost, so retrying would duplicate the exact thing this endpoint exists to prevent. A batch that fails is reported in `failed` with `reason: 'batch-failed'`; batches that already succeeded stay in `added` (partial success, not all-or-nothing). Batches are sequential, never parallel, to avoid a `snapshot_id` race. A `404` from Spotify on read or write is translated to `404 SPOTIFY_PLAYLIST_NOT_FOUND`, and clears `SpotifyAccount.defaultPlaylistId` only if the playlist that 404'd was the current default.

**Two deviations from the spec, found while implementing:** (1) `/v1/search` track objects no longer carry `popularity` or `preview_url` — confirmed against the live API. Both default gracefully (`?? 0` / `?? null`), so nothing breaks, but `popularityNudge` is inert in practice today. (2) The spec's `-0.05` "explicit mismatch" scoring penalty was dropped entirely — the source song object (`{ title, artist, duration?, confidence? }`) never carries an `explicit` field to compare against, so there is nothing to detect a mismatch with.
